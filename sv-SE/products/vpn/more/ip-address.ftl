...
# Outdated string
vpn-ip-address-no-your-ip = Nej. Din IP-adress är bara kopplad till en plats om du inte <a href="{ $url }">använder ett VPN</a> (vi kommer mer in på det senare). När du är hemma och ansluter till internet som du betalar för använder du en. Men om du kontrollerar din e -post hemma på morgonen, sedan skannar nyheterna på ett lokalt kafé medan du väntar på ditt kaffe och sedan arbetar från ett kontor, har du använt olika IP-adresser på varje plats.
...
#   $mozvpn (url) - https://www.mozilla.org/products/vpn/
#   $countries (number) - number of countries where Mozilla VPN has servers, e.g. "30". The + after indicates the number may be higher, where "30+" means "30 or more".
vpn-ip-address-a-vpn-is = Ett VPN är ett sätt att dölja din IP-adress. <a href="{ $vpn }">När du använder ett VPN</a> kommer din externa IP-adress från VPN-serverns externa IP-adress, istället för din plats. Så om din anslutande VPN-server ligger i Kalifornien kommer din externa IP-adress att se ut som att den är ansluten från Kalifornien, oavsett var du faktiskt befinner dig. Dessutom skickas din onlineaktivitet via en krypterad, säker anslutning till din VPN-server, vilket ger dig ytterligare säkerhet och integritet. <a href="{ $mozvpn }">{ -brand-name-mozilla-vpn }</a> är ett sätt att dölja din IP-adress. Vi lagrar inte aktivitetsloggar eller samarbetar med analysplattformar från tredje part. Vi erbjuder fullständigt enhetsskydd för upp till fem enheter med servrar i { $countries } länder. Du kan ansluta till var som helst, från var som helst.
