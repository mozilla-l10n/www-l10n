...
privacy-faq-we-are-big-believers-of-data = We are big believers of data minimisation and not asking for things we don't need.
# Variables:
#   $accounts (url) - link to https://support.mozilla.org/kb/firefox-options-preferences-and-settings
privacy-faq-you-dont-need-an-account-to = You don't need an account to use { -brand-name-firefox }. <a href="{ $accounts }">Accounts</a> are required to synchronise data across devices, but we only ask you for an email address. We don't want to know things like your name, address, birthday and phone number.
...
privacy-faq-we-do-ask-our-advertising-partners = We do ask our advertising partners to use only first party data that web sites and publishers know about all users, such as the browser you are using and the device you are on.
