...
privacy-faq-v2-we-are-big-believers = We are big believers of data minimisation and not asking for things we don't need.
# Variables:
#   $attrs (url) - link to https://www.mozilla.org/account/
privacy-faq-v2-you-dont-need-an-account = You don’t need an account to use { -brand-name-firefox }. <a { $attrs }>Accounts</a> are required to synchronise data across devices, but we only ask you for an email address and your age (just to make sure you’re not a child).
...
privacy-faq-v2-we-do-not-ask = We do ask our advertising partners to use only first party data that web sites and publishers know about all users, such as the browser you are using and the device you are on.
