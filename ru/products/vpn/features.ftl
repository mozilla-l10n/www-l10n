...
vpn-features-stronger-malware = Более надёжная защита от вредоносных программ
...
vpn-features-with-the-mozilla-vpn-extention = С помощью расширения { -brand-name-mozilla-vpn } для { -brand-name-firefox } (только Windows) вы можете настроить работу VPN для каждого веб-сайта. Исключайте отдельные веб-сайты из защиты VPN или устанавливайте предпочитаемые местоположения серверов для определённых сайтов, что делает работу с ними более гибкой и персонализированной.
vpn-features-personalized-server = Персональные рекомендации по местоположению серверов
vpn-features-well-suggest-which-servers = Мы подскажем, какие серверы рядом с вами обеспечат самое быстрое и надёжное подключение к Интернету.
vpn-features-personalize-which-apps = Выберите, какие приложения будут получать VPN-защиту
vpn-features-easily-exclude-apps = Легко исключайте приложения из VPN-защиты — не нужно отключать своё устройство от { -brand-name-mozilla-vpn }. Доступно на устройствах Windows, Android и Linux.
...
vpn-features-were-backed-by-mofo-v2 = Нас поддерживает <a { $mofo }>{ -brand-name-mozilla-foundation }</a>, некоммерческая организация, которая борется за то, чтобы Интернет был открытым и здоровым для всех людей.
