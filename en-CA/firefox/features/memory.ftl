...
features-memory-chrome-uses-up-to-177x-more = <a href="{ $url }">{ -brand-name-chrome } uses up to 1.77x more memory than { -brand-name-firefox }</a>. If your computer is already low on memory, this can cause a significant slowdown. Using { -brand-name-firefox }’s latest version with multi-process can result in more available memory to run your favourite programs.
features-memory-browse-faster-privately = Browse faster, privately
# Variables:
#   $url (url) - link to https://www.mozilla.org/firefox/features/private-browsing/
# "Decluttering" means reducing clutter
features-memory-explore-the-web-faster-with = Explore the web faster with <a href="{ $url }">{ -brand-name-firefox } Private Browsing</a>. Only { -brand-name-firefox }’s private mode includes tracking protection which blocks ads with trackers from loading on pages. De-cluttering sites means web pages can load faster.
