...
vpn-ip-address-this-is-a = To je trochu zašmjatane prašenje – wotmołwa je haj kaž tež ně. Wjacore graty móža samsnu eksternu (zjawnu) IP-adresu dźělić, ale kóždy grat ma swójsku lokalnu (priwatnu) IP-adresu. Waš internetny poskićowar (ISP) na přikład wašu domiznu z jednej eksternej IP-adresu zarjaduje. Dokelž so waš router poprawom z internetom zwjazuje, so IP-adresa wašemu routerej připokaza. Waš router potom kóždemu gratej, kotryž je internetom zwjazany, lokalnu IP-adresu připokazuje. Eksterna IP-adresa so z wonkownym swětom dźěli. Waša lokalna IP-adresa so zwonka wašeje priwatneje syće doma njedźěli.
