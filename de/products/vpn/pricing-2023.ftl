...
vpn-pricing-access =
    { $servers ->
        [one] Zugriff auf { $servers } Server in über { $servers } Ländern
       *[other] Zugriff auf { $servers } Server in über { $servers } Ländern
    }
...
vpn-pricing-get-annual-subscription = Holen Sie sich ein Jahresabo
...
vpn-pricing-the-first-time-you = Wenn Sie das erste Mal { -brand-name-mozilla-vpn } über { -brand-name-mozilla }s Website abonnieren und Ihr Konto innerhalb der ersten 30 Tage kündigen, können Sie eine Rückerstattung beantragen und { -brand-name-mozilla } erstattet Ihnen die Kosten für den ersten Abozeitraum.
...
vpn-pricing-we-adhere-strictly = Wir halten uns strikt an die <a { $principles }>Datenschutzprinzipien</a> von { -brand-name-mozilla }. Wir sammeln die Daten, die wir brauchen, um das { -brand-name-mozilla-vpn } funktional zu erhalten das Produkt mit der Zeit zu verbessern. Außerdem tracken wir Kampagnen- und Empfehlungsdaten in unserer mobilen App, um { -brand-name-mozilla-vpn } zu helfen, die Effektivität unserer Marketing-Kampagnen nachzuvollziehen. Erfahre mehr dazu in unserem <a { $notice }>Datenschutzhinweis</a>.
