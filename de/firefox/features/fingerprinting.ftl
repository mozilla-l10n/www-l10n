...
features-fingerprinting-the-practice-of = Durch Fingerprinting können Sie <a { $url_tracked }>monatelang getrackt werden</a>, selbst wenn Sie Ihren Browserspeicher löschen oder den privaten Surf-Modus verwenden – auch, wenn Sie klargestellt haben, dass Sie nicht getrackt werden möchten. Trotz der nahezu vollständigen Übereinkunft zwischen <a { $url_standards }>Standards,</a> <a { $url_bodies }>Behörden</a> und <a { $url_browser }>Anbietern</a> von <a { $url_vendors }>Browsern,</a> dass Fingerprinting <a { $url_harmful }>schädlich ist</a>, <a { $url_has }>hat</a> dessen Nutzung im Internet im letzten Jahrzehnt <a { $url_steadily }>kontinuierlich</a> <a { $url_increased }>zugenommen.</a>

# $url_details (string) - https://blog.mozilla.org/security/2020/01/07/firefox-72-fingerprinting/
features-fingerprinting-the-latest-firefox = Der neuste { -brand-name-firefox }-Browser schützt Sie vor Fingerprinting, indem Anfragen von Drittanbietern an Unternehmen, die dafür bekannt sind, Fingerprinting anzuwenden, blockiert werden. Wir haben hart daran gearbeitet, diesen Datenschutz zu ermöglichen, ohne die Websites zu beschädigen, die Sie besuchen möchten. (Lesen Sie hier mehr, wenn Sie sich für die <a { $url_details }>technischen Einzelheiten</a> interessieren.)

# $url_protections (string) - https://www.mozilla.org/firefox/privacy/products/
features-fingerprinting-and-its-not = Die Einstellung ist einfach zu finden. Im neuesten { -brand-name-firefox }-Browser wird Fingerprinting automatisch blockiert. Besuchen Sie Ihr <a { $url_protections }>Datenschutz-Dashboard</a>, um zu sehen, wie Sie heimlich getrackt werden und wie { -brand-name-firefox } dies verhindert.

# $url_download (string) - link to firefox/new
features-fingerprinting-you-probably-wouldnt = Wahrscheinlich möchten Sie im wahren Leben auch nicht auf Schritt und Tritt verfolgt werden. Es gibt also keinen Grund, das online zu akzeptieren. Falls Sie nicht schon { -brand-name-firefox } haben, <a { $url_download }>laden Sie ihn herunter und schützen Sie sich selbst</a> vor digitalem Fingerprinting.
