...
#   $mozvpn (url) - https://www.mozilla.org/products/vpn/
#   $countries (number) - number of countries where Mozilla VPN has servers, e.g. "30". The + after indicates the number may be higher, where "30+" means "30 or more".
vpn-ip-address-a-vpn-is = Un VPN es una forma de ocultar tu dirección IP. <a href="{ $vpn }">Cuando usas un VPN</a>, tu dirección IP externa provendrá de la IP externa del servidor VPN, en lugar de la dirección IP externa de tu ubicación. Así que si tu servidor VPN de conexión está ubicado en California, tu IP externa dirá que te conectaste desde California, sin importar dónde estés. Además, tu actividad en línea se envía a través de una conexión cifrada y segura a tu servidor VPN, dándote seguridad y privacidad adicionales. <a href="{ $mozvpn }">{ -brand-name-mozilla-vpn }</a> es una forma de ocultar tu dirección IP. No guardamos registros de actividad ni nos asociamos con plataformas de análisis de terceros. Ofrecemos protección de dispositivos completos para hasta cinco dispositivos con servidores en más de { $countries } países, puedes conectarte a cualquier lugar, desde cualquier lugar.
