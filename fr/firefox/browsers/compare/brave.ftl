...
# HTML page title
compare-brave-firefox-vs-brave-which-is = { -brand-name-firefox } ou { -brand-name-brave } : quel est le meilleur navigateur pour vous ?
...
compare-brave-there-are-a-few-of-braves = Quelques fonctionnalités de sécurité dans { -brand-name-brave } méritent d’être soulignées, comme ses mises à niveau de connexions HTTPS automatiques (que { -brand-name-firefox } propose également via une <a { $extension }>extension</a> ). { -brand-name-brave } et { -brand-name-firefox } offrent tous les deux aux utilisateurs un <a { $lockwise }>gestionnaire de mots de passe natif</a> et la possibilité de vérifier leurs statistiques de sécurité à tout moment. { -brand-name-brave } affiche des statistiques comme le nombre de traqueurs qu’il a bloqués chaque fois que vous ouvrez un nouvel onglet. { -brand-name-firefox } affiche des informations similaires lorsque vous consultez <a { $privacy }> votre rapport de confidentialité</a>, accessible à tout moment en cliquant sur le bouclier dans la barre d’adresse.
# "Basic Attention Token" is a brand name.
compare-brave-brave-also-recently-gained = Recemment, { -brand-name-brave } est également devenu capable de synchroniser les données ainsi que les Basic Attention Tokens entre les systèmes d'exploitation et les appareils les plus répandus.
