...
privacy-passwords-most-logins-today = 目前大部分的登入資訊都有密碼保護。如果攻擊者知道您的密碼他們就可以存取您的帳號並做任何您能用該帳號做的事情。因此當您問您的帳號能有多安全時您應該問的是密碼能有多安全。這代表您必須想好要讓攻擊者透過各種方式取得您的帳號密碼
...
privacy-passwords-to-keep-your = 為了確保登入安全您需要盡可能多地封鎖這些登入資訊。每種風險都有不同的緩釋方式。
privacy-passwords-look-for-the = 尋找瀏覽器中的鎖頭
privacy-passwords-its-easy-to = 很簡單就能防止攻擊者在您未加密的網站登入時偷走您的密碼: 如果您沒有在網址列看到鎖形圖示請在輸入密碼前三思而後行。
privacy-passwords-a-closed-lock = 在所有主要瀏覽器中網址列中的網址前面都會出現一個關閉的掛鎖。
