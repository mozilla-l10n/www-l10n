...
mobile-compare-since-your-mobile = 由於您的行動瀏覽器是隨時隨地都能取得資訊的生命線速度、安全性、隱私權和易用性是獲得良好體驗的關鍵。哪套行動瀏覽器是最棒的讓我們來比較一下頂尖的播放器 — 看看哪套最符合您的需求。
...
mobile-compare-a-main-ingredient = 一個好用的瀏覽器的一個主要因素是它的功能。有什麼好處幾乎五套瀏覽器在分頁、書籤、欄位自動完成功能方面都差不多但只有 { -brand-name-firefox } 和 { -brand-name-safari } 提供了附加元件擴充套件。贊讓您的瀏覽器更隱私、更強大或更有趣。
# The strong tag wraps a word that gets highlighted with a colorful underline for emphasis. If the translation requires multiple words, please choose a word to emphasize and wrap that word in the strong tag.
...
mobile-compare-almost-all-of = 比較型的瀏覽器大部分都允許在桌面版與行動版之間完成完整的同步。
mobile-compare-the-good-news = 對 { -brand-name-iphone } 使用者來說好消息是{ -brand-name-apple } 最近允許 { -brand-name-iphone } 使用者選擇他們自己的預設瀏覽器這樣就可以同步裝置與舉例來說他們在 { -brand-name-iphone } 或 { -brand-name-windows } 電腦上安裝了 { -brand-name-firefox }。
mobile-compare-conclusion = 結論
mobile-compare-based-on-the = 以我們勾勒出的標準隱私、功能、同步來說只有 { -brand-name-firefox } 能同時符合這三點。 { -brand-name-firefox } 有類似在功能與同步分類上的功能但在隱私保護上卻與眾不同因為我們覺得這是最重要的元件讓您可以只用自己喜歡的方式享受上網。
mobile-compare-our-recommendations-aside = 撇開我們的建議不談但究竟究竟要取決於您的需求以及在上網時最看重哪套瀏覽器。
mobile-compare-search-engine-options = 搜尋引擎選項
mobile-compare-text-to-speech = 文字轉語音
mobile-compare-reader-mode = 閱讀器模式
mobile-compare-spell-checking = 拼字檢查
mobile-compare-web-extensionsaddons = Web extension / 附加元件
mobile-compare-dark-mode = 暗色模式
mobile-compare-desktop-mode = 桌面版模式
mobile-compare-find-on-page = 在頁面中搜尋
...
mobile-compare-send-a-download-link-to-your = 傳送下載鏈結到您的手機。
mobile-compare-scan-the-qr-code-to-get-started = 掃描 QR Code 開始使用
mobile-compare-scan-alt-text = 掃描 QR Code，下載 { -brand-name-firefox } 到您的行動裝置
