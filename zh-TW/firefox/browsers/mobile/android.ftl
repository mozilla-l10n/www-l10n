...
mobile-android-get-to-private = 只要輕鬆一點即可進入隱私瀏覽模式。當您關閉隱私瀏覽模式時也會自動清除裝置中的瀏覽紀錄。
mobile-android-search-your-own = 用您的方式搜尋
mobile-android-got-a-big = 手機要大又正常嗎沒問題。 { -brand-name-firefox } for { -brand-name-android } 讓您可以將搜尋列從上到下移動方便單手就能使用。您也可以在瀏覽器中選擇自己使用的搜尋引擎也可以將 { -brand-name-firefox } 設為預設瀏覽器。
...
mobile-android-got-lots-of = 有很多開啟的分頁嗎沒問題把這些照片整理到收藏集以繼續完成處理。收藏集還可以跨裝置分享讓您不論使用哪台裝置都可以繼續瀏覽相同的東西。
mobile-android-supercharge-firefox-with = 透過附加元件，加強 { -brand-name-firefox } 的相關功能
mobile-android-addons-are-like = 附加元件就像是您可將其安裝到瀏覽器中透過增加預設隱私權設定、客制化您的使用體驗等強大的小工具。
