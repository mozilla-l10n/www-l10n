...
features-fingerprinting-every-fingerprint = 每個指紋都是唯一的。
features-fingerprinting-download-firefox = 下載 { -brand-name-firefox }
...
features-fingerprinting-the-practice-of =
    就算您清除掉瀏覽器的儲存空間或是使用隱私瀏覽模式，您可能還是會被<a { $url_tracked }>追蹤好幾個月</a> — 而完全不理會您明確表達不想被追蹤的意象。儘管Despite a near complete
    agreement between <a { $url_standards }>標準規範</a><a { $url_bodies }>組織</a>以及<a { $url_browser }>瀏覽器</a><a { $url_vendors }>廠商</a>都近乎同意數位指紋追蹤是<a { $url_harmful }>有害的</a>，過去十年來，它在網路上的用量<a { $url_has }>仍然</a><a { $url_steadily }>穩定</a><a { $url_increased }>成長</a>。
