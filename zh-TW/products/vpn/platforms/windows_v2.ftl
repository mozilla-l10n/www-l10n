...
vpn-windows-protect-your-windows-v2 = 使用 VPN 保護您的 Windows 裝置。{ -brand-name-mozilla-vpn } 是由非營利公司在背後全力相挺。了解 VPN 如何保護您，以及為什麼您應該信任 { -brand-name-mozilla } 能保護您的隱私權與安全。
...
vpn-windows-a-vpn-creates = VPN 透過在您的 Windows 電腦與網際網路之間建立加密過的「通道」來運作，並對網站隱藏您的實際位置。這個功能可防止第三方與網路間諜，了解太多不需要知道的資訊。
...
vpn-windows-if-youre = 若您是線上影音串流的重度使用者，使用 VPN 可以避免電信業者限制頻寬或限制流量。
vpn-windows-you-can-exclude = 您可以透過分割通道功能，讓某些應用程式不經由 VPN 連線。
vpn-windows-using-a-windows = 在公開、未加密的 Wi-Fi 網路使用 Windows 裝置，會讓您的網路流量更容易被網路間諜追蹤。而 VPN 可加入更強大的保護，協助您確保資料隱私。
