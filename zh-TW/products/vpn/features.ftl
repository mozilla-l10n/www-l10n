...
vpn-features-webste-specific-vpn = 與 { -brand-name-firefox } 無縫整合的網站 VPN 設定
vpn-features-with-the-mozilla-vpn-extention = 您可以透過 { -brand-name-mozilla-vpn } 的 { -brand-name-firefox } 擴充套件（僅有 Windows 版）針對每個網站調整 VPN 使用體驗。可將特定網站排除 VPN 保護，或設定偏好的伺服器位置，讓您有更靈活、更個人化的使用體驗。
...
vpn-features-were-backed-by-mofo-v2 = 我們是由致力於保持 Web 對所有人的開放與健康的非營利組織 <a { $mofo }>{ -brand-name-mozilla-foundation }</a> 所支持的。
