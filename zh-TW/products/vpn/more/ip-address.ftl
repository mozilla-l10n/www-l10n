...
vpn-ip-address-you-dont-need = 您不需要隱藏 IP 地址但某些時候您可能會想要隱藏。最常見的原因是隱私權。在美國，<a href="{ $congress }">國會否決了</a>旨在保護寬頻使用者隱私的隱私權規定。網路服務供應商會知道您的上網習慣、用來做什麼、以及在每個頁面停留的時間。此通訊內容並未加密第三方可看到您正造訪的網站。解決這個問題的一個方法是 <a href="{ $dho }">DNS-over-HTTPS</a> (DoH)。此功能會加密您的 DNS 網域流量讓 ISP 更難看到您開啟的網站。美國的 <a href="{ $firefox }">{ -brand-name-firefox } 使用者</a>預設情況下您的 DoH 查詢將會傳送到受信任的 DNS 伺服器讓您更難偵測到哪些網站造訪。
# Variables
#   $url (url) - https://www.mozilla.org/products/vpn/more/when-to-use-a-vpn/
