...
# "Screenshots" is a brand name when capitalized and plural, but also appears here as a regular, non-brand term (lowercase and singular).
compare-safari-screenshots-is-another-popular = <a { $attrs }>截图</a>是 { -brand-name-firefox } 浏览器内置的另一项广受欢迎的功能，可让您轻松捕获屏幕图像。截图时，您可以选择复制文件或存放到文件夹，不会弄乱桌面。
...
compare-safari-firefox-also-offers-a-similar-updated-v3 = { -brand-name-firefox } 也提供了类似的同步功能，只需您创建一个免费的 <a %(attrs)s>{ -brand-name-mozilla-account }</a>，便可以轻松地在多台设备间同步书签、浏览历史、首选项、密码、表单信息、附加组件和最近打开的标签页（25 个）。{ -brand-name-firefox } 与 Safari 的不同之处在于，它可以在任何桌面或移动平台——iOS、Android、Windows、Linux 或 macOS 上皆可使用，跨平台性绝佳。
# Obsolete string
# Variables:
#   $attrs (string) - link to /firefox/accounts/ with additional attributes for analytics
