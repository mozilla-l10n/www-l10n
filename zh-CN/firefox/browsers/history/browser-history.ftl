...
browser-history-ncsa-mosaic-ran = NCSA Mosaic 易于使用，又可以在 { -brand-name-windows } 计算机运行，让任何拥有 PC 的人都可以浏览器早期的网页、聊天室、图库。隔年（1994 年），安德森创办了 <a href="{ $netscape }">{ -brand-name-netscape }</a> 并公开发行了 { -brand-name-netscape } Navigator。这是第一款面向普通用户的浏览器，取得了巨大成功。 这也是针对互联网用户的新型战争的开端。
browser-history-the-browser-wars = 浏览器大战
# Variables:
#   $ie (url) - https://en.wikipedia.org/wiki/Internet_Explorer
browser-history-by-1995-netscape = 来到 1995 年，{ -brand-name-netscape } Navigator 已不是上网冲浪的唯一选择。计算机软件巨头 { -brand-name-microsoft } 获得了旧版 Mosaic 的代码授权，而后建立了自己的“Web 窗口”—— <a href="{ $ie }">{ -brand-name-ie }</a>。该浏览器的推出引发了战争。{ -brand-name-netscape } 与 { -brand-name-microsoft } 竭力为各自的产品推出新版本，试图以更快、更好的产品来超越对方。
# Variables:
...
browser-history-today-there-are = { -brand-name-firefox }、{ -brand-name-google } { -brand-name-chrome }、{ -brand-name-microsoft } { -brand-name-edge }、{ -brand-name-safari } 與 { -brand-name-opera } 是市场中主要的竞争者。在过去的十年中，移动设备已成为访问互联网的首选方式。 目前，大多数用户只使用移动浏览器与<a href="{ $applications }">应用程序</a>来上网。主流浏览器也都提供了适用于 { -brand-name-ios } 及 { -brand-name-android } 设备的移动版本。尽管这些应用程序在特定用途上相当好用，但只提供了受限的上网功能。
# Variables:
#   $vr (url) - https://mixedreality.mozilla.org/
browser-history-in-the-future-the = 在未来，Web 可能会跳脱原始的超文本，提供各式不同而广阔的互动式体验。虚拟现实已经出现了几十年（从 1992 年的《割草者（Lawnmower Man）》 与 1995 年的任天堂 Virtual Boy 算起），但 Web 最可能将其带入大众。{ -brand-name-firefox } 已经支持 <a href="{ $vr }">WebVR 与 A-Frame</a>，可让开发者快速便捷地构建虚拟现实网站。大多数现代移动设备都支持 <a href="{ $vr }">WebVR</a>，可以将手机插入简单的纸盒中，变成 VR 头戴设备。由科幻小说家尼尔·斯蒂芬森（Neal Stephenson）所描绘的 3D 虚拟现实 Web 世界可能已经离我们不远。如果是这样的话，网页浏览器可能会融入为另一个世界的真实入口。
