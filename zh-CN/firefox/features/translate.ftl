...
features-translate-one-of-the-best-things-about = 互联网最棒的特点，在于将五湖四海的内容联结到了一起。不管是新闻、博客，还是最新数码产品的评测，在近乎无边无际的网络上全都能找到。{ -brand-name-firefox } 最新的翻译功能可以全程自动翻译网页。
features-translate-while-other-browsers-rely-on = 其他浏览器的翻译功能均依靠云服务实现，而 { -brand-name-firefox } 浏览器会将“{ -brand-name-firefox } 翻译”语言模型下载到用户的浏览器中，并在本地完成翻译，因此 { -brand-name-mozilla } 并不记录您所翻译的网页。
features-translate-when-you-translate-a-webpage = 翻译网页，隐私无忧
features-translate-when-your-translations-are = 在本地进行翻译时，数据完全不会离开您的设备，也无需依靠云服务进行翻译。这样一来，即便是 { -brand-name-mozilla } 也不知道您所翻译的网页，这也是我们的翻译功能有别于其他翻译工具的显著特点。
