...
vpn-or-proxy-weve-done-the = 我们已经做了大量工作以确保 <a href="{ $mozvpn }">{ -brand-name-mozilla-vpn }</a> 和 <a href="{ $fpn }">{ -brand-name-firefox-private-network }</a> 代理扩展实际上尊重您的隐私，这是我们愿意以我们的声誉为赌注的东西。 { -brand-name-mozilla } 以构建可帮助您保护信息安全的产品而闻名。我们遵循易于阅读、严肃的<a href="{ $principles }">数据隐私原则</a>，这使我们能够只关注提供服务所需的信息。由于我们得到了一家<a href="{ $mission }">使命驱动</a>公司的支持，您可以相信，您为该产品花费的金钱不仅可以确保您拥有一流的安全性，但也让互联网更适合每个人。
