...
features-adblocker-its-important-to-note = É importante ter em consideração que alguns sites confiam nos anúncios para carregar conteúdo ou fornecer funcionalidades. Bloquear anúncios pode fazer com que algumas funcionalidades apresentem problemas, tais como vídeos que não são reproduzidos ou opções de início de sessão que falham. Escolha um bloqueador de anúncios que seja atualizado regularmente e se encontrar um bloqueador de anúncios que causa problemas significativos, tente outro para encontrar um melhor equilíbrio entre desempenho e compatibilidade.
...
features-adblocker-tool-1 = <strong>Modo Padrão:</strong> Uma opção balanceada que bloqueia rastreadores comuns enquanto mantém um funcionamento suave do site.
features-adblocker-tool-2 = <strong>Modo Estrito:</strong> Ideal para utilizadores que pretendem uma proteção da privacidade mais forte, ao bloquear mais rastreadores e cookies. Note que alguns sites podem necessitar de ajustes neste modo.
features-adblocker-tool-3 = <strong>Modo Personalizado:</strong> Para aqueles que pretendem controlo total, o modo Personalizado permite-lhe selecionar o que bloquear, desde cookies a rastreadores e mais.
features-adblocker-to-adjust = Para ajustar estas definições, clique no ícone de escudo à esquerda da barra de endereço em qualquer página web e selecione “Definições de Proteção.” Isto irá abrir um menu onde pode escolher o nível adequado de proteção para si.
features-adblocker-why-use = Porquê Usar um Bloqueador de Anúncios?
features-adblocker-ad-blockers-do-more = Os bloqueadores de anúncios fazem mais do que gerir anúncios – eles podem melhorar a sua velocidade de navegação e segurança ao reduzir conteúdo desnecessário. Estes permitem-lhe criar uma experiência web à medida, e sem distrações, permitindo que se foque no que mais importa.
# Variables:
