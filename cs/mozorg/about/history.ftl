...
history-within-the-first-year-new = Během <a href="{ $firstyear }">prvních let</a> noví členové komunity z celého světa začali přispívat a vytvářet nové funkce, vylepšovat existující a zapojovat se do řízení a plánování projektu samotného.
# Variables:
#   $stevecase (url) link to https://www-archive.mozilla.org/stevecase.html
history-by-creating-an-open-community = Vytvoření otevřené komunity se projekt { -brand-name-mozilla } stal <a href="{ $stevecase }">větší než jakákoliv jednotlivá společnost</a>.
# Variables:
...
history-community-members-got-involved = Členové komunity se zapojili a rozšířili dosah <a href="{ $mission }">původní mise</a> – místo práce na dalším prohlížeči { -brand-name-netscape } lidé začali vytvářet <a href="{ $browsers }">mnoho prohlížečů</a>, <a href="{ $bugzilla }">nástrojů pro vývojáře</a> a dalších <a href="{ $projects }">projektů</a>.
history-people-contributed-to-mozilla = Lidí přispívali do projektu { -brand-name-mozilla } různými způsoby, ale každý je zapálený do tvorby svobodného softwaru, který by všem umožnil vybrat si, jak prožívat internet.
# Variables:
...
history-it-did-this-by-continuing = Toho dosáhla pokračováním vydávání software jako je { -brand-name-firefox } a { -brand-name-thunderbird } a objevováním nových oblastí jako poskytování <a href="{ $grants }">grantů</a> na podporu přístupnosti na webu.
# Variables:
...
history-more-people-than-ever-before = Více lidí než kdy dříve používají internet a užívají si jej <a href="{ $all }">v jejich vlastním jazyce</a>.
# Variables:
#   $range (url) link to https://www.wikipedia.org
#   $areas (url) link to https://creativecommons.org/
history-a-sustainable-organization = Byla vytvořena udržitelná organizace, která využívá tržní mechanismy pro podporu veřejného zájmu. Tento model byl znovu použit dalšími k vytvoření otevřených, transparentních a spolupracujících organizací v <a href="{ $range }">širokém spektru</a> <a href="{ $areas }">mnoha oblastí</a>.
history-the-future-is-full-of-challenges = Budoucnost je plná výzev a příležitostí, které se rovnají těm v naší minulosti.
